...
{    if ( from_[x].size( ) != 1 || to_[x].size( ) != 1 )
     {    std::cout << "Problem in JoinEdges.\n";
          PRINT(x);
          std::cout << "edges in = " << printSeq( ToEdgeObj(x) ) << std::endl;
          std::cout << "edges out = " << printSeq( FromEdgeObj(x) ) << std::endl;    }
...
{    digraph::Initialize( from, to );
     ...
     {    if ( used[i] > 1 || ( !allow_unused_edges && used[i] == 0 ) )
          {    std::cout << "Edge " << i << " is used " << used[i]
...
{    if ( d < 0 ) return False;
     if ( v == w ) return True;
     std::set< std::pair<int,F> > unprocessed, processed;

     unprocessed.insert( std::make_pair( v, 0 ) );
     while( !unprocessed.empty( ) )
     {    int x = unprocessed.begin( )->first;
          F dx = unprocessed.begin( )->second;
          typename std::set< std::pair<int,F> >::iterator u 
               = processed.lower_bound( std::make_pair( x, 0 ) );
          ...
          {    int y = From(x)[j];
               ...
               if ( dy > d ) continue;
               if ( y == w ) return True;
               typename std::set< std::pair<int,F> >::iterator p 
                    = processed.lower_bound( std::make_pair( y, 0 ) );
               if ( p != processed.end( ) && p->first == y )
               {    if ( p->second <= dy ) continue;
                    processed.erase(p);    }
               typename std::set< std::pair<int,F> >::iterator u 
                    = unprocessed.lower_bound( std::make_pair( y, 0 ) );
...
template<class F>
void digraphE<F>::GetSuccessors( const vec<int>& v, vec< std::pair<int,F> >& from_v )
{    std::set< std::pair<int,F> > check, fromv;

     for ( int i = 0; i < v.isize( ); i++ )
          check.insert( std::make_pair( v[i], 0 ) );
     while( !check.empty( ) )
     {    int x = check.begin( )->first;
          F dx = check.begin( )->second;
          typename std::set< std::pair<int,F> >::iterator u 
               = fromv.lower_bound( std::make_pair( x, 0 ) );
          check.erase( check.begin( ) );
          if ( u != fromv.end( ) && u->first == x )
          {    if ( u->second <= x ) continue;
               fromv.erase(u);    }
          fromv.insert( std::make_pair( x, dx ) );
          for ( int i = 0; i < From(x).isize( ); i++ )
          {    int y = From(x)[i]; 
               F dy = dx + EdgeObjectByIndexFrom( x, i );
               typename std::set< std::pair<int,F> >::iterator a 
                    = check.lower_bound( std::make_pair( y, 0 ) );
               if ( a != check.end( ) && a->first == y )
               {    if ( a->second <= dy ) continue;
                    check.erase(a);    }
               typename std::set< std::pair<int,F> >::iterator b 
                    = fromv.lower_bound( std::make_pair( y, 0 ) );
               if ( b != fromv.end( ) && b->first == y )
               {    if ( b->second <= dy ) continue;
                    fromv.erase(b);    }
               check.insert( std::make_pair( y, dy ) );    }    }
     from_v.clear( );
     for ( typename std::set< std::pair<int,F> >::iterator i = fromv.begin( ); 
          i != fromv.end( ); ++i )
     {    from_v.push_back(*i);    }    }
...
void PrintEdge( const int v, const int w, const int ei, const vec<double>& lengths,
     ...
     const typename digraphE<F>::edge_label_info eli, std::ostream& out )
{
...
void PrintEdge2( const int v, const int w, const int ei, const vec<double>& lengths,
     const vec<Bool>* dashed, const vec<String>* edge_color, const int tiny_top,
     const typename digraphEX<F>::edge_label_info eli, std::ostream& out )
{
     float wd = 0.1; // this value not used
     String color, label;
     Bool bold = False;
...
template<class E> void LabelTransitionVertices( const digraphE<E>& G,
     const int v, const vec<Bool>* invisible, std::ostream& out )
...
void DotHeader( const Bool label_contigs, const Bool label_vertices,
     ...
     const double aspect, std::ostream& out )
{    out << "digraph G {\n\n";
     if (label_vertices)
...
digraphE<F>::PrettyDOT( std::ostream& out, const vec<double>& lengths,
...
template<class F> void
digraphEX<F>::PrettyDOT( std::ostream& out, const vec<double>& lengths,
     const edge_label_info eli, Bool label_contigs, Bool label_vertices,
     const vec<int>* componentsToPrint, const vec<String> *label_contigs_extra,
     const vec<int> *verticesToPrint, const vec<Bool>* dashed,
     const vec<Bool>* invisible, const vec<String>* edge_color,
...
template<class E> 
     void Compare( std::ostream& out, const digraphE<E>& g1, const digraphE<E>& g2 )
{    if ( g1.N( ) != g2.N( ) )
          std::cout << "first graph has " << g1.N( ) << " vertices but "
               << "second graph has " << g2.N( ) << "\n";
     if ( g1.From( ) != g2.From( ) ) std::cout << "from_ not the same\n";
     if ( g1.To( ) != g2.To( ) ) std::cout << "to_ not the same\n";
     if ( g1.Edges( ) != g2.Edges( ) ) std::cout << "edges_ not the same\n";
     if ( g1.ToEdgeObj( ) != g2.ToEdgeObj( ) )
          std::cout << "to_edge_obj_ not the same\n";
     if ( g1.FromEdgeObj( ) != g2.FromEdgeObj( ) )
          std::cout << "from_edge_obj_ not the same\n";
     if ( g1 != g2 ) std::cout << "DIGRAPHS ARE NOT EQUAL\n";
     return;    }
     ...
     vec<Bool>& complete, const int max_paths )
{
     ...
     {    vec< std::pair< vec<int>, int > > 
               paths( { std::make_pair( vec<int>({v}), 0 ) } );
          while( paths.isize( ) <= max_paths )
          {    vec< std::pair< vec<int>, int > > paths2;
               for ( const auto& p : paths )
               {    int x = p.first.back( );
                    vec< std::pair<int,int> > ext;
                    for ( int j = 0; 
     ...
     const int max_paths )
{
     ...
     for ( int pass = 1; pass <= 2; pass++ )
     {
          ...
          #pragma omp parallel for  shared(hanging)
          for ( int v = 0; v < G.N( ); v++ ) {
            const vec<int>& V = ( pass == 1 ? G.From(v) : G.To(v) );
