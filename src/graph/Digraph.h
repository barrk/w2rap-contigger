...
class digraph
{
     ...
     void DOT( std::ostream& out, const vec<String>& vertex_colors ) const;
     void DOT( std::ostream& out, const vec<String>& vertex_colors,
          const vec< vec<String> >& edge_colors ) const;
     void DOT( std::ostream& out, const vec< vec<String> >& edge_labels ) const;
     void PrettyDOT( std::ostream& out, Bool label_contigs = True, 
     ...
     void DOT_vl( std::ostream& out, const vec<String> & vertex_labels,
          const String& layout = "", 
     ...
     void DOT_el( std::ostream& out, const String& layout = "", 
     ...
     void WriteGraphML( std::ostream& out, const vec< vec<String> >& edge_labels ) const;
...
template<class F> class digraphE : public digraph {
     ...
     void GetSuccessors( const vec<int>& v, vec< std::pair<int,F> >& from_v );
     ...
     void DOT_vel( std::ostream& out, const vec<String> & vertex_labels );
     ...
     void PrettyDOT( std::ostream& out, const vec<double>& lengths,
...
template <> void digraphE<int>::DOT_vel( std::ostream& out, 
...
template<class F> bool operator==( const digraphE<F>& g1, const digraphE<F>& g2 );
template<class F> 
     void Compare( std::ostream& out, const digraphE<F>& g1, const digraphE<F>& g2 );
...
template<class T> class digraphE_V1 {
     ...
     digraphE_V1( const int N, const std::function< vec<std::pair<int,T>> (int)>& from )
     {    N_ = N;
          from_ = from;    }

     int N( ) const { return N_; }

     vec< std::pair<int,T> > FromVec( const int v ) const
     {    return from_(v);    }
     ...
     int N_;
     std::function< vec< std::pair<int,T> > (int) > from_;
...
template<class E> class EmbeddedSubPath {
     ...
     {    ForceAssertGt( a.size( ), 0 );
          ...
          {    int v = a[i], w = a[i+1];
               ...
               if ( !found )
               {    std::cout << "No edge found from vertex " << v << " to vertex "
                         << w << ".\n";
                    ForceAssert( 0 == 1 );    }    }    }
