...
void HyperBasevector::PrintSummaryDOT0w( std::ostream& out, Bool label_contigs,
...
Bool HyperBasevector::TestValid( const Bool exit ) const
{
     ...
     {    if ( EdgeLengthKmers(e) <= 0 )
          {    std::cout << "HyperBasevector edge " << e << " has length "
                    << EdgeLengthKmers(e) << " kmers." << std::endl;
  ...
  for ( int v = 0; v < N(); v++ ) {
    ...
    {    std::cout << "\nProblem in HyperBasevector at vertex " << v 
         ...
         {    if ( j > 0 ) std::cout << ",";
              ...
              std::cout << e << "[l=" << EdgeLengthKmers(e) << "]";    }
         std::cout << " and exiting edges ";
...
bool ComponentsAreIsomorphic( const HyperBasevector& hkp1, int seed_vx_1,
  ...
  vec< std::pair<int,int> > hypotheses;
  ...
  hypotheses.push_back( std::make_pair(seed_vx_1,seed_vx_2) );
  ...
  while( ! hypotheses.empty() ) {
    ...
    for( size_t i=0; i < from_v1.size(); i++ )
      hypotheses.push_back( std::make_pair(from_v1[i],from_v2[i]) );
    for( size_t j=0; j < to_v1.size(); j++ )
      hypotheses.push_back( std::make_pair(to_v1[j],to_v2[j]) );
...
template void digraphE<BaseVec>::PrettyDOT( std::ostream& out, 
...
template void digraphEX<BaseVec>::PrettyDOT( std::ostream& out, 
     const vec<double>& lengths, const edge_label_info, Bool label_contigs, 
     Bool label_vertices, const vec<int>* componentsToPrint,
     const vec<String> *label_contigs_extra, const vec<int>* verticesToPrint,
     const vec<Bool>* dashed, const vec<Bool>* invisible,
...
template Bool digraphE<BaseVec>::EdgePaths(int, int, vec<vec<int> >&, const int, const int, const int) const;
...
{    int64_t x = 0;
     ...
     {    const basevector& b = EdgeObject(e);
          for ( int j = 0; j < b.isize( ); j++ )
               x += ( b[j] + 1 ) * ( e + 1 ) * ( j + 1 );    }
     x += ((digraphE<basevector>&)(*this)).CheckSum( );
     return x;    }
