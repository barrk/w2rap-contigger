...
void ReftraceVariants(std::ostream& out, std::ostream& callsOut, const vecbasevector& G, 
        ...
        const HyperBasevector& hbp, const vec<std::pair<int,Bool>>& to_hb,
...
struct Caller {
    ...
    friend std::ostream& operator<<(std::ostream& out, const Caller& var) {
...
struct VariantCall {
    Variant variant; 
    Caller caller;  
    friend std::ostream& operator<< (std::ostream& out, const VariantCall& vc) {
...
public:
    ...
    void AddBranch(const vec<int>& path, const vec<VariantCall>& vcalls, 
            const align& a, const std::pair<int,int>& trim, vec<double> weight = vec<double>()) 
    ...
    std::pair<int,int> GetTrim(int branch_id) const 
    ...
    // ================== Output ==================================
    void PrintTable(TextTable& tb, std::ostream& callsOut, int& index, int& block_id,
            ...
            size_t max_index, const std::map<Variant, vec<std::pair<double,double>>> probs
        ) const;
...
private:
    ...
    vec<std::pair<int,int>> trims_;            // branch was trimmed 
