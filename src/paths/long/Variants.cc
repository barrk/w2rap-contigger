...
void ReftraceVariants(std::ostream& out, std::ostream& callsOut, const vecbasevector& G, 
        ...
        const HyperBasevector& hbp, const vec<std::pair<int,Bool>>& hbp_to_hb,
        const vec<vec<vec<std::pair<int,int>>>>& ref_pos_all_s, const vec<vec<vec<int>>>& best_path_s,
        ...
        const long_logging* logc)
{
    ...
    std::map<Variant, vec<std::pair<int,int>>> var_friending;
...
void VariantCallGroup::PrintTable(TextTable& tb, std::ostream& callsOut, int& index, int& block_id,
        const vec<std::pair<int,Bool>>& hbp_to_hb, 
        const vec<String> ref_tags, const vec<size_t> ref_start,
        const std::map<Variant, vec<std::pair<int,int>>>& var_friending,
        size_t max_index, const std::map<Variant, vec<std::pair<double,double>>> probs) const
{
    const bool PrintPathProb =  ( ref_weight_.size() && weights_.size() );  // print, if we have weights
    const size_t MaxSeqLen = 10;
    std::map<Variant, std::set<int>> variant_callers; 
    std::map<Variant, vec<double>> variant_prob;
    ...
    for (auto it = variant_callers.begin(); it != variant_callers.end(); ++it) {
        ...
        for (size_t k = 0; k < branches_.size(); k++) {
            if(pathids.find(k) == pathids.end()){
                ...
                for (const VariantCall& x: vcall_vec) {
                    ...
                    if (std::max(var_start, var_start2) < std::min(var_end, var_end2)) {
                        overlap = true;
                        break;
                    }
                }
        ...
        if (it2 != var_friending.end()) {
            const vec<std::pair<int,int>>& friend_locs = it2->second;
...
void VariantCallGroup::FillVCF(VCFWriter& vcf
                              ...
                              ,const std::map<Variant, vec<std::pair<int,int>>>& var_friending
                              ,const std::map<Variant, vec<std::pair<double,double>>> probs
