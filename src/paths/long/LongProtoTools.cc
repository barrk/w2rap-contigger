...
{    int64_t gsize = 0;
     ...
     std::cout << "-[" << std::setiosflags(std::ios::fixed) << std::setprecision(0) << excess_edges_per
          << std::resetiosflags(std::ios::fixed) << "]- excess edges per Mb in EFASTA" 
          << " (total excess edges in EFASTA = " << excess_edges << ")" << std::endl;
     if (PERF_STATS)
...
{    std::cout << "\n" << Date( ) << ": done, time used = " << TimeSince(clock)
          << ", peak mem used = " << std::setiosflags(std::ios::fixed) << std::setprecision(1)
...
{    vec< triple<int,ho_interval,int> > x;
     ...
     for ( int id = 0; id < (int) reads.size( ); id++ )
     {    if ( x[id].first >= 0 )
          {    std::cout << x[id].third << "   " << x[id].first << "."
                    << x[id].second << "\n";    }    }    }
...
{    double hours = double( WallClockTime( ) - clock ) / 3600.0;
     ...
     std::cout << "\n" << Date( ) << ": done, time used = " << TimeSince(clock)
          << ", peak mem used = " << std::setiosflags(std::ios::fixed) << std::setprecision(1)
          << PeakMemUsageGB( ) << std::resetiosflags(std::ios::fixed) << " GB" << std::endl;    }
