...
void ComputeHomopolymerProb_AT_Diploid( vec<std::tuple<uint64_t,double,uint64_t>>& n_q_len){
    ...
    for(uint64_t ff=0;ff<nFlavors;++ff){
        ...
        nReads+=nread_loc;
        p_from_q[ff] = (nread_loc>0) ? (1.0-pow(10.0 , -0.1*std::get<1>(n_q_len[ff]))): 0.0;

        const double P_D = 0.01 * std::max( 1.0 , 0.4 * (length-10.0) + 1.0 ) ;
        const double P_I = 0.01 * std::max( 0.25 , 0.1 * (length-10.0) + 0.25 ) ;
...
void AdjustHomopolymerProb(unsigned char baseval, std::unordered_map< uint64_t , vec<std::tuple<uint64_t,double,int>>>& length_lookup){
    ...
    for( const auto&len_entry: length_lookup){
        ...
        n_q_len.push_back( std::make_tuple(count,qsum,len_entry.first));
...
//        if(!bProceed ) continue;
        ...
        vec<vec<std::map<Variant,std::pair<bool,bool> > > > hinges(nBranches,vec<std::map<Variant ,std::pair<bool,bool> >> (nSamples));

        for (auto it = variant_branches.begin(); it != variant_branches.end(); ++it) { // for each variant
            const Variant& var = it->first;
            const std::set<int>& pathids = it->second;
        ...
        basevector& edge_extended, int& len_extended_left) 
{
    ...
    int trim_left = std::max(0, ext_left - ext_max);
        ...
        vec<std::tuple<int,int,read_place>> edge_rid_place) 
{
    ...
    for (size_t i = 0; i < edge_rid_place.size(); i++) {
        int e = -1, rid = -1;
        read_place rp;
        std::tie(e, rid, rp) = edge_rid_place[i];
        vec<int> rshifts;
    ...
    for (size_t i = 0; i < edge_rid_place.size(); i++) {
        ...
        for (size_t k = i; k < j; k++) {
            int rid = std::get<1>(edge_rid_place[k]);
            ...
            for (size_t j = i+1; j < lens.size(); j++) {
                ...
                if ((int)head.size() > ext) {
                    ...
                    if (short_extended == bases[j]) {
                        ...
                        max_ext_left = std::max(max_ext_left, ext + more_ext);
                    }
                }
                if ((int)tail.size() > ext) {
                    ...
                    if (short_extended == bases[j]) {
                        ...
                        max_ext_right = std::max(max_ext_right, ext + p2);
                    }
                }
            }
        }
        ...
        const long_logging* logc) 
{
    ...
    for (size_t grpid = 0; grpid < vcall_groups.size(); grpid++) {
        ...
        for (int branchid = 0; branchid < group.GetNBranch(); branchid++) {
            ...
            for (const VariantCall& x: vcalls)
                var_group_branch[x.variant].insert(std::make_pair( grpid, branchid ));
        }
    }
    ...
    for (size_t i = 0; i < overlapping_vars.size(); i++) {
        ...
        std::tie(index1, index2, g, start, end) = overlapping_vars[i];
    ...
    for (size_t i = 0; i < clusters.size(); i++) {
        ...
        for (auto it = grouped.begin(); it != grouped.end(); ++it) {
            const std::map<int, vec<int>>& branches = (*it).second;
...
// designed according to the no-change-of-results requirement established on Sept 4, 2013.
    ...
    std::map<int,vec<int>>  edge_to_varid;
    for (size_t cid = 0; cid < clusters.size(); cid++) {
        int prev_g = (cid == 0 ? -1 : std::get<2>(clusters[cid-1]));
        int next_g = (cid == clusters.size()-1 ? -1 : std::get<2>(clusters[cid+1]));
        int g, start, end;
        std::tie(std::ignore, std::ignore, g, start, end) = clusters[cid];
        ...
        if (g == next_g && std::get<3>(clusters[cid+1]) > end) {
            basevector ref_edge(Gplus[g],  end, std::get<3>(clusters[cid+1]) - end);
            variant_graph.AddVertices(1);
            ConnectLastTwoVertices(variant_graph, edge_to_varid, ref_edge, vec<int>());
        }
    ...
    std::map<int, vec<std::pair<uint64_t,uint64_t>>> read_support_count;
    std::map<int, vec<std::tuple<int,int,int,int>>> read_support_by_strand;
    for (int n1 = 0; n1 < variant_graph.N(); n1++) {
        ...
        // for + and - read dir
        vec<vec<std::pair<int,int>>> nreads_by_strand(nbranches, vec<std::pair<int,int>>(nsamples));
        ...
        if (verbosity >= 2) {
            ...
            for (size_t j = 0; j < branches.size(); j++) {
                ...
                for (int sample = 0; sample < nsamples; sample++)
                    std::cout << " nreads= " << nreads[j][sample] << " q= " << qsum[j][sample];
                std::cout  << std::endl;
            }
        }
            ...
            for (int sample = 0; sample < nsamples; sample++) {
                ...
                for (size_t j = 0; j < branches.size(); j++) {
                    if (nreads[j][sample] > 0 && nreads[j][sample] <= WeakSupport &&
                        ...
                        weak.push_back(j);
                        max_weak_support = std::max(max_weak_support, qsum[j][sample]);
                    }
                }
...
//std::cout << "bayo: vid=" <<vid << " sample="<<sample << " " << std::get<0>(ans) << " " << std::get<1>(ans) << " " << std::get<2>(ans) << " " << std::get<3>(ans) << " " << std::get<4>(ans) << " " << std::get<5>(ans) << " " << std::get<6>(ans) << " " << std::get<7>(ans) << std::endl;
                probs[vars[vid]][sample].first += std::get<0>(ans);
                probs[vars[vid]][sample].second += std::get<1>(ans);
                read_support_count[vid][sample].first += std::get<2>(ans);
                read_support_count[vid][sample].second += std::get<3>(ans);
                std::get<0>(read_support_by_strand[vid][sample]) += std::get<4>(ans);
                std::get<1>(read_support_by_strand[vid][sample]) += std::get<5>(ans);
                std::get<2>(read_support_by_strand[vid][sample]) += std::get<6>(ans);
    ...
    if (verbosity >= 1){
        ...
        for (size_t vid = 0; vid < vars.size(); vid++) {
            ...
            for(int sample = 0; sample < nsamples; sample++) {
                ...
                std::tie(ref_plus, ref_minus,var_plus, var_minus)
        ...
        std::ofstream ofs("strand_bias.txt");
        tb.Print(ofs, 1, "rrrrr");
    }

    if (DetectSingleEdge) {
        const double QsumRefSingleEdge = 0;
        std::set<Variant> var_on_single_edges;
        for (size_t grpid = 0; grpid < vcall_groups.size(); grpid++) {
            const VariantCallGroup& group = vcall_groups[grpid];
...
*/
    ...
    if (verbosity >= 1)
        std::cout << Date() << ": Finding the best placement for each read " << std::endl;
    ...
    if (verbosity >= 1)
        std::cout << Date() << ": Finding the read support and qsum difference " << std::endl;
    ...
    for (size_t idx = 0; idx < ids.size(); idx++ ) {   
        ...
        for ( int i = 0; i < v.isize( ); i++ ) {
            ...
            for (int e2: e2s) group_best_qsum = std::min(group_best_qsum, best_qsums[e2]);
    ...
    if (verbosity>=2) {
        ...
        std::cout << "read support for each edges " << std::endl;
        for (size_t e = 0; e < nreads.size(); e++) {
            std::cout << "edge" << e << " nreads=" << nreads[e] << ": ";
