     ...
     {    vec<String> logging;
          ...
          {    if ( !m[j].Contains( ":" ) )
               {    std::cout << "\nI'm sorry, the VERB option " << m[j]
                         << " does not contain a colon, as required." << std::endl;
                    exit(1);    }
...
LoggingBool( DIRECT, False, "send main output to std::cout directly, runs as single "
...
LoggingBool( MIN_LOGGING, True, "generate at least minimal logging" );
     ...
     {    if ( !used[i] )
          {    std::cout << "\nIllegal logging " << logging[i] << "." << std::endl;
               fail = True;    }    }
     if (fail)
