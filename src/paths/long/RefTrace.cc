        ...
        const EdgePlacements& edge_placements, const basevector& Gg) 
{
    ...
    for (size_t i = 1; i < best_path.size(); i++) {
        if (to_left[best_path[i]] != to_right[best_path[i-1]]) {
            ...
            max_gap_bases = std::max(max_gap_bases, ref_start - last_stop);
        } 
    }
...
void FindPositionOnRef( vec<vec<vec<int>>>& best_path_s,
              vec<vec<vec<std::pair<int,int>>>>& ref_pos_all_s,
              ...
              std::ostream& out)
{
    if (! HasGoodCoverage(best_path2[best_g], eids2[best_g], edge_placements, 
        ...
        out << "Best path has low quality. Try alternative "
            "penalty function that do not over-penalize gaps." << std::endl;
        GraphZ gz2(edge_placements, &PenaltyForVariantCalling);
        vec< vec<int> > spaths2; 
        vec< triple<int,int,int> > spaths_egd2;
        vec< std::pair<int,int> > spaths_gg_pen2;
        int best_g2;
    ...
    vec<vec<std::pair<int,int>>> ref_pos_all(gsize); 
...
{    
    ...
    const vec<int>& Gplus_ext = ref.Gplus_ext;
     ...
     int K = hb.K( );
     if ( verbosity >= 2 )
          out << "assembly has " << hb.EdgeObjectCount( ) << " edges" << std::endl;
     ...
     if ( verbosity >= 1 ) out << Date( ) << ": building hbp" << std::endl;
     HyperBasevector hbp; 
     vec<std::pair<int,Bool>> hbp_to_hb;
     ...
     vec< std::pair<int,int> > spaths_gg_pen;
     ...
     {    double gclock = WallClockTime( );
          ...
          if ( spaths.empty( ) )
          {    out << "BEST_GLOBAL_OUT: there are no paths, giving up." << std::endl;
               _exit(1);    }
          ...
          if (logc.REFTRACE_VARIANTS) {
              ...
              vec<vec<vec<std::pair<int,int>>>> ref_pos_all_s(G.size());
     ...
     if(logc.REFTRACE_VARIANTS_SUMMARY){
         out << penalty << " penalty (p = 1*e + 100*g + 0.1*d)" << std::endl;
         out << "-[" << std::setiosflags(std::ios::fixed) << std::setprecision(2) << errors_per_Mb
              << std::resetiosflags(std::ios::fixed) << "%%%]- errors"
              << " (e = errs = " << total_errs << ")" << std::endl;
         if (logc.SHOW_REFTRACE_EVENTS)
              out << "-[" << meta_events << "]- error meta-events" << std::endl;
         out << "-[" << std::setiosflags(std::ios::fixed) << std::setprecision(2) << gaps_per_Mb
              << std::resetiosflags(std::ios::fixed) << "]- excess gaps per Mb "
              << "(g = total gaps = " << total_gaps
              << ", \"excess\" = " << total_gaps - free_gaps << ")" << std::endl;
...
RefTraceResults RefTrace( const ref_data& ref,
     const HyperBasevector& hb, const vec<int>& inv, 
     const int verbosity, const long_logging& logc, 
     std::ostream& out, RefTraceHeuristics rth,
     const String& BEST_GLOBAL_OUT, const Bool fix_bug)
{
    NullOStream nullOS;
