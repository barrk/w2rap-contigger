...
struct EdgeLoc{
    ...
    EdgeLoc(const std::pair<int,int>& edge_start) :original_edge(edge_start.first), start(edge_start.second) {}
};
...
public:
    ...
    //
    void InitFromBestPath(const vec<int>& path, const vec<std::pair<int,int>>& ref_pos) ;
