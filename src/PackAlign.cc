                      ...
                      const avector<int>& gaps, const avector<int>& lengths )
{
  ...
  /* // XXX
     ...
     {
     std::cout << "packalign constructor failure\n"; // XXX
                    ...
                    const vecbasevector& EE, int test_no, Bool fatal )
{
  ...
  std::cout << "actual errors = " << std::flush;
  if ( !rc2 ) std::cout << ActualErrors( EE[id1], EE[id2], a ) << "\n";
  else 
  {
    basevector EErcid2 = EE[id2];
    EErcid2.ReverseComplement( );
    std::cout << ActualErrors( EE[id1], EErcid2, a ) << "\n";    
  }
  std::cout << "gaps/lengths:" << std::flush;
  for ( int i = 0; i < a.Nblocks( ); i++ )
    std::cout << " " << a.Gaps(i) << "/" << a.Lengths(i);
  std::cout << "\n";
  PRINT(test_no);
...
void align::Read( std::istream& in, int& errors, int& id1, int& id2, Bool& rc )
{
...
align align::TrimmedTo1(const int start, const int len) const {
  ...
  while (true) {
	...
	if (!onGap) {
	  ...
	  //std::cout << "added gap " << gaps.size()-1 << "of size " << gaps.back() << std::endl;
        }
	else {
	  blocks.push_back(Lengths(block));
	  //std::cout << "added block " << blocks.size()-1 << "of size " << blocks.back() << std::endl;
        }
      }
      if (!onGap) {
        ++block; 
	...
	&& !onGap) {
      ...
      startOn1 = p1;
      gaps.push_back(0);
      //std::cout << "Added gap "<< gaps.size()-1<<" with size "<<gaps.back()<<std::endl;
      blocks.push_back(currentSize - local);
      //std::cout << "Added block " << blocks.size()-1<<" with size "<<blocks.back()<<std::endl;
      ret.pos1_ = p1;
      ret.pos2_ = p2;
      //std::cout << "starting at " << (onGap ? "gap " : "block ")
      //<< block << " position " << local << " p1 is " << p1 << std::endl
      //<< "currentSize " << currentSize << std::endl;
	...
	if (onGap) {
      ...
      //std::cout << "ending at " << (onGap ? "gap " : "block ")
      //<< block << " position " << local << " p1 is " << p1 << std::endl;
      break;
    }
    //std::cout << "at: " << (onGap ? "gap " : "block ")
...
std::vector<int> align::MutationsGap1Gap2( const basevector& rd1, 
                                      const basevector& rd2 ) const {
  std::vector<int> answer(3, 0);
  int p1 = pos1( ), p2 = pos2( );
  for ( int j = 0; j < Nblocks( ); j++ ) {
    if ( Gaps(j) > 0 )  {
      answer[1] += Gaps(j);
...
std::pair<int, int> align::Gap1Gap2( ) const {
...
void align::PrintMutations( const basevector& rd1, const basevector& rd2, std::ostream& log, const bool zero_based) const
{
  int answer = 0, j, p1 = pos1( ), p2 = pos2( );
  int loc1 = zero_based ? p1 : p1+1;
  int loc2 = zero_based ? p2 : p2+1;
  for ( j = 0; j < Nblocks( ); j++ )
  {
    if ( Gaps(j) > 0 ) {
	...
	log << loc1 << " " << Base::val2Char(rd1[p1])
	    << " " << loc2 << " " << Base::val2Char(rd2[p2]) << std::endl;
      }
      ++p1; 
      ++p2;
      ++loc1;
...
void align::Write( std::ostream& out, int id1, int id2, Bool rc, int errors )
{
  BinWrite( out, pos1_ );
  BinWrite( out, pos2_ );
  BinWrite( out, errors );
