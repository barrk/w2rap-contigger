...
class align {
 ...
 public:
  ...
  void Read( std::istream& in, int& errors, int& id1, int& id2, Bool& rc );

  ///Not correlated with Read
  void Write( std::ostream& out, int id1, int id2, Bool rc, int errors );
  ...
  Bool GapFree( ) const { return Gap1Gap2( ) == std::make_pair( 0, 0 ); } 
...
class placement_mark {
     ...
     friend std::ostream& operator<<( std::ostream& out, const placement_mark& m )
